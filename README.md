![Feelio](https://github.com/user-attachments/assets/6f6c12f9-915b-4ecc-9556-87c67b98ca62)

<div align="center">
 <h3><b>Feelio</b></h3>
AI 감정분석 다이어리 서비스<br><br>
  2025.05.07 ~ 2025.05.23
  <br><br>

  [Notion](https://friendly-sternum-f92.notion.site/19da6b30f99980ad8530e8e65396a025?pvs=74)  |  [UCC]()

</div>
<br/>

## 서비스 배경 💡
현대 사회는 빠르게 변화하고 있으며, 그 속에서 많은 사람들은 감정의 기복과 정서적인 피로감을 겪고 있습니다. 특히 자신의 감정을 말로 표현하거나 주변에 털어놓는 것이 어려운 사람들에게, 일기와 같은 개인적인 기록은 감정을 정리하고 돌아보는 중요한 수단이 됩니다.

하지만 단순히 일기를 쓰는 것만으로는 감정의 흐름이나 패턴을 인식하기 어렵고, 스스로 감정을 객관적으로 바라보는 데 한계가 있습니다. 이에 따라, AI 기술을 활용해 개인의 감정을 분석하고 피드백을 제공하는 정서 관리 서비스에 대한 수요가 증가하고 있습니다.

AI 감정분석 다이어리 서비스는

- 사용자가 작성한 일기 내용을 바탕으로 감정 상태를 분석하고,
- 감정의 흐름을 시각적으로 보여주며,
- AI와의 대화를 통해 위로를 받거나 정서적 교감을 경험할 수 있도록 지원합니다.

이 서비스는 단순한 기록을 넘어서,

- 사용자의 감정과 활동을 트래킹하고
- 정서적인 성장과 자기 인식을 도와주는 디지털 파트너의 역할을 지향합니다.

누구나 부담 없이 감정을 털어놓고, 스스로를 이해하며 더 건강한 하루를 만들어갈 수 있도록 돕는 것이 이 서비스의 핵심 목표입니다.
<br><br><br>

## 서비스 이름 및 설명 🖥
Feelio는 매일의 감정을 기록하고, AI를 통해 분석하며, 자신을 조금씩 더 이해해가는 감정 분석 기반 다이어리 서비스입니다. 단순한 일기장을 넘어, 감정을 정리하고 트래킹하며, 필요할 땐 AI 친구와 대화를 통해 위로를 받을 수 있는 감정 케어 플랫폼을 지향합니다.
<br><br><br>

## 개발 환경 및 기술 스택 🔨

### FE
`HTML`  <br>
`CSS`  <br>
`JavaScript` <br>
`JQuery` <br>
`Thymeleaf` <br><br>


### BE
`Java 17`  <br>
`Spring Boot 3.1`  <br>
`Gradle` <br>
`JPA`  <br>
`Oracle 18c` <br>
`Swagger2` <br><br>


### CI/CD
`AWS EC2`  <br>
`Docker`  <br>
`Docker-compose` <br><br>


### 버전/이슈 관리

`Git` <br>
`Github` <br><br>


### TOOL

`Postman` <br>
`Excalidraw` <br><br>


### 협업

`Discord` <br>
`Notion` <br><br>


### IDE

`Visual Studio Code` <br>
`InteliJ` <br>
`STS4` <br><br>


<br><br>

## 서비스 구조도 🔍
<img width="750" alt="image" src="https://user-images.githubusercontent.com/60915285/188927329-3cf2a1a5-1e58-4896-9163-2049ab7cfa5f.png">

<br><br>

## 주요 기능 📚

### 1. 소셜로그인
<img width="500" src="https://github.com/user-attachments/assets/0ae09287-7e50-45d3-a7f2-b4568a439fc4"><br>
로컬, 네이버, 구글, 카카오 소셜 로그인을 제공합니다.

<br><br>
        
### 2. 홈화면
<img width="300" src="https://github.com/user-attachments/assets/0a4ec1f7-51ca-42d4-8f22-b5ab895e904a"><br>
사용자가 작성한 일기가 표시됩니다.

<br><br>

### 3. 일기 작성 및 분석
<img width="700" src="https://user-images.githubusercontent.com/60915285/189544287-de42403c-4c30-48b5-93bf-dbb20486d092.gif">
추천하고싶은 여행 경로를 사용자들에게 추천할 수 있습니다. <br>
GooGle API를 통해 정확한 위치를 제공하며 Pin과 경로 표시를 통해 효율적인 여행 경로를 확인할 수 있습니다.

<br><br>

### 4. 마이페이지
<img width="700" src="https://user-images.githubusercontent.com/60915285/189544280-2bf99a41-5bda-40bd-b92e-63248777c0c1.gif">
비슷한 취미나 성향을 가진 사람끼리 테스트를 할 수 있습니다.<br>

<br><br>

### 5. 차트
<img width="700" src="https://user-images.githubusercontent.com/60915285/189544280-2bf99a41-5bda-40bd-b92e-63248777c0c1.gif">
비슷한 취미나 성향을 가진 사람끼리 테스트를 할 수 있습니다.<br>

<br><br>

### 6. AI채팅
<img width="700" src="https://user-images.githubusercontent.com/60915285/189544288-b9496177-6e94-4ed5-a95b-9f2f0904e9c1.gif">
방의 모든 참가자들은 채팅 기능을 사용할 수 있습니다.<br>
여기에는 비속어 필터링이 존재하여, 비속어가 섞인 채팅을 보내게 되면 '[삭제된 메세지]' 라고 대체되어 보내집니다.

<br><br>



## 프로젝트 산출물 📝
- [와이어프레임(Figma)](https://www.figma.com/file/ePhUEm8n0WOKCVpdS3znoY/Untitled?node-id=10%3A34)
- [요구사항 명세서](https://docs.google.com/spreadsheets/d/1xPJsyiMqSk_K8tLmQkwHjnBlbztizLL380-utx67S0Y/edit?usp=sharing)
- [API 명세서](https://docs.google.com/spreadsheets/d/1DsIVlvEm14OrcFZ1Z_UtRkTFgNZIzEe8x1uVugoBL0o/edit?usp=sharing)
- [ERD](https://www.erdcloud.com/d/qgQJ5vEiGkjDeJmgy)
- [회의록](https://sunset-offer-934.notion.site/caac1e153e834de1aa797a7745dbd0a1)
- [데일리 스크럼](https://sunset-offer-934.notion.site/74a1a2fd88574e98a9b18b3a0547d4c8)
- [포팅 메뉴얼](https://github.com/Cheer-Together/Cheer-Together/blob/main/exec/01_%EA%B3%B5%ED%86%B5PJT_%EB%8C%80%EC%A0%842%EB%B0%98_B204_%EB%B9%8C%EB%93%9C_%EB%B0%8F_%ED%8F%AC%ED%8C%85_%EB%AC%B8%EC%84%9C.pdf)
- [외부 API 문서](https://github.com/Cheer-Together/Cheer-Together/blob/main/exec/02_%EA%B3%B5%ED%86%B5PJT_%EB%8C%80%EC%A0%842%EB%B0%98_B204_%EC%99%B8%EB%B6%80_API_%EC%82%AC%EC%9A%A9.pdf)
- [시연 시나리오](https://github.com/Cheer-Together/Cheer-Together/blob/main/exec/04_%EA%B3%B5%ED%86%B5PJT_%EB%8C%80%EC%A0%842%EB%B0%98_B204_%EC%8B%9C%EC%97%B0_%EC%8B%9C%EB%82%98%EB%A6%AC%EC%98%A4.pdf) 


<br><br>
## 개발 문화 💁‍♂️
- 데일리 스크럼을 매일 진행
- 간단한 회의부터 중요한 회의, 모든것을 노션에 기록하는 습관 📝
- 의견은 자유롭게!
- 재밌게, 함께 성장하는 개발하기
- 머지 리퀘스트 열심히 작성하고 서로 코드 리뷰 
<img width="800" alt="스크린샷 2022-10-13 오전 4 03 17" src="https://user-images.githubusercontent.com/60915285/195427955-51e7b2b8-ec0f-4225-8d29-63e458df37ba.png">
<img width="800" alt="스크린샷 2022-10-13 오전 4 04 08" src="https://user-images.githubusercontent.com/60915285/195427670-f1d7b0f1-4e36-4554-953a-2c00c075c7f2.png">
<img width="600" alt="스크린샷 2022-10-13 오전 4 04 35" src="https://user-images.githubusercontent.com/60915285/195427335-866764a7-b3ba-4423-ac88-384ec78a42eb.png">



<br><br>
## 팀원 소개 👪
<table>
    <tr>
        <td height="140px" align="center"> <a href="https://github.com/Qulip">
            <img src="https://avatars.githubusercontent.com/ParkJuseung" width="140px" /> <br><br> 박주승  </a> <br></td>
        <td height="140px" align="center"> <a href="https://github.com/josephkimmm">
            <img src="https://avatars.githubusercontent.com/josephkimmm" width="140px" /> <br><br> 김두현 </a> <br></td>
        <td height="140px" align="center"> <a href="https://github.com/dnjs0">
            <img src="https://avatars.githubusercontent.com/dnjs0" width="140px" /> <br><br> 박세원 </a> <br></td>
        <td height="140px" align="center"> <a href="https://github.com/jern2">
            <img src="https://avatars.githubusercontent.com/jern2" width="140px" /> <br><br> 이지원 </a> <br></td>
    </tr>
</table>

<br><br><br>

